...
class timingc_v3_eq(Beta):
    ...
    def process(self):
        ...
        factor7 = pd.read_pickle('./dump/timing7.pkl')
        beta = (factor1 + factor2 + factor3 + factor4 + factor5 + factor6 + factor7)/7
        self.signal_df = beta#.apply(lambda col: pd.cut(col, bins=[-1, -0.5, -0.2, 0.2, 0.5, 1], labels=[-1, -0.5, 0, 0.5, 1], include_lowest=True)).astype(float)